...
MyGame = ig.Game.extend({
	...
	update: function() {
		...
		if(monster.reachEnd && arrow.arrowStop){
				if(!do_throwing){
					monster.currentAnim = monster.anims.throwing.rewind();
					do_throwing = true;
				}
				if(!ball.fly && monster.currentAnim.loopCount > 0){
		...
		console.log(monster.currentAnim.loopCount);
